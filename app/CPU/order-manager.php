...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        //     $shipping_method_id = $data['shipping_id'];
        // }
        ...
        $shipping_cost = CartManager::get_shipping_cost($data['cart_group_id']);
        $orderAmount = CartManager::cart_grand_total($cart_group_id);
        if (isset($data['delivery_charge']) && $data['delivery_charge'] != "") {
            $shipping_cost = $data['delivery_charge'];
            $orderAmount = CartManager::cart_grand_total($cart_group_id) - CartManager::get_shipping_cost($data['cart_group_id']);
        }
        ...
        $or = [
            ...
            'order_amount' => $orderAmount - $discount,
        ...
        $startDate = $orderDate->copy()->addDay()->format('d-m-Y');
        $endDate = $orderDate->copy()->addDays(2)->format('d-m-Y'); 

        $or['expected_delivery_date_range'] = $startDate . ' - ' . $endDate;
        ...
        self::add_order_status_history($order_id, auth('customer')->id(), $data['payment_status'] == 'paid' ? 'pending' : 'pending', 'customer');

        $stat = $data['payment_status'] == 'paid' ? 'pending' : 'pending';

        $notify_user = [
            'title' => 'New order placed (Order Id ' . $order_id . ')',
            'description' => $data['payment_status'] == 'paid' ? 'Your order placed success.' : 'Your order placed success. Wait for vendor confirmation',
            'type' => 'order',
            'order_id' => $order_id . '-' . $stat,
            'user_id' => $user->id
        ];
        self::save_user_notification($notify_user);

        $sellerNotification = [
            'title' => 'New order received',
            'description' => 'Received new order (Order Id ' . $order_id . ')',
            'type' => 'order',
            'type_id' => $order_id,
            'user_id' => $seller_data->seller_id
        ];
        self::save_seller_notification($sellerNotification);
    ...
    public static function send_notification($fcm_token)
    {
        $data = [
            'title' => translate('order'),
