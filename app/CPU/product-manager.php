...
class ProductManager
{
    ...
    {
        // $paginator = Product::with(['reviews'])->where('status', 1)->orderBy('id', 'desc')->take(10)->get();
        $paginator = Product::active()->with(['rating', 'tags', 'seller.shop'])->where('status', 1)->orderBy('id', 'desc')->paginate($limit, ['*'], 'page', $offset);
        /*$paginator->count();*/
        return [
    ...
    public static function get_product_for_you($limit = 10, $offset = 1)
    {
        $product_ids = ProductForYou::pluck('product_id')->toArray();
        ...
        $paginator = Product::active()->with(['rating', 'tags', 'seller.shop'])
            ->where('status', 1)
    ...
    public static function get_featured_products($limit = 10, $offset = 1)
    {
        ...
        $paginator = Product::active()->with(['rating', 'tags'])->active()
            ->where('featured', 1)
    ...
    public static function search_products_web($name, $category = 'all', $limit = 10, $offset = 1)
    {
        ...
        $product = Product::active()->with(['rating', 'tags'])->where('name', 'like', "%{$name}%")->first();

        if (!empty($product)) {
            $paginator = Product::active()->with(['rating', 'tags'])->where('name', 'like', "%{$name}%");
        } else {
            $paginator = Product::active()->with(['rating', 'tags'])->where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->orWhere('name', 'like', "%{$value}%")
    ...
    public static function translated_product_search_web($name, $category = 'all', $limit = 10, $offset = 1)
    {
        ...
        $paginator = Product::with('tags')->WhereIn('id', $product_ids);
    ...
    public static function get_all_products($limit = 10, $offset = 1, $params = [], $sort = [], $filters = [], $productids = [])
    {

        $product = Product::with(['rating', 'tags']);
        ...
        if (!empty($filters)) {
            if (isset($filters['min_price']) && $filters['min_price'] != "" && isset($filters['max_price']) && $filters['max_price'] != "") {
                // $product->whereBetween('unit_price', [$filters['min_price'], $filters['max_price']]);
                $product->whereBetween('unit_price', [Helpers::convert_currency_to_usd($filters['min_price']), Helpers::convert_currency_to_usd($filters['max_price'])]);
            }

            if (isset($filters['review_filter']) && $filters['review_filter'] != "") {
                $product->where('reviews_count', '>=', $filters['review_filter']);

                $product->where('rating', '>=', $filters['review_filter']);
            }

            if (isset($filters['search_text']) && $filters['search_text'] != "") {
                $textArray = explode(' ', $filters['search_text']);
                if (!empty($textArray)) {
        ...
        if (!empty($sort)) {
            if ($sort['sort_by'] == 'price') {
                $sort['sort_by'] = 'unit_price';
            }

            if ($sort['sort_by'] == 'review') {
                $sort['sort_by'] = 'reviews_count';
            }

            $product->orderBy($sort['sort_by'], $sort['order_by']);
        }

        $paginator = $product->latest()->paginate($limit, ['*'], 'page', $offset);
