...
class CartManager
{
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        if ($request->has('is_cart') && $request->input('is_cart') == 0) {
        ...
        //         ->where('seller_is', $product->added_by)->where('product_id', $product->id)->first();
        // } else {
        ...
        // generate group id for same seller/admin product one cart
        if ($user == 'offline') {
            $check = session('offline_cart');
            $cart_check = $check
        ...
        $shippings = Helpers::get_shipping_methods(1, 'admin');

        $cart['customer_id'] = $user->id ?? 0;
        $cart['quantity'] = $request['quantity'];
        $data['shipping_method_id'] = $shippings[0]->id ?? "";
        $cart['price'] = $price;
        $cart['tax'] = $tax;
        ...
        if ($request['is_cart'] == 0) {
            session()->put('buy_now', $cart['cart_group_id']);
        }
        ...
        if ($user == 'offline') {
            ...
            if (isset($cart->product_id)) {
                if (Savelist::where(['product_id' => $cart->product_id, 'customer_id' => $user->id])->delete()) {
