...
class HomeController extends Controller
{
    ...
    public function default_theme()
    {
        ...
        $brands = Brand::forStorefrontHome(15);
        //best sell product
        ...
        if (auth('customer')->check()) {
            ...
            if ($orders) {
                ...
                $topProductIds = DB::table('order_details')
                    ...
                    ->orderByDesc('total')
                    ->limit(10)
                    ->pluck('product_id'); // returns an array of top 10 product IDs

                $topProducts = $this->product->with([
                    'wish_list' => function ($query) {
                        return $query->where('customer_id', Auth::guard('customer')->user()->id ?? 0);
                    },
                    'compare_list' => function ($query) {
    ...
    public function alpha_ecom()
    {
        $current_date = date('Y-m-d H:i:s');
        $topProducts = collect();
        $newlyAddedProducts = collect();
        $home_categories = $this->category
            ->where('home_status', true)
            ->priority()->get();
        ...
        if (auth('customer')->check()) {
            ...
            if ($orders) {
                $orders = $orders?->map(function ($order) {
                    ...
                    $order['id'] = $order_details[0]->id ?? 0;
                    $order['category_id'] = $order_details[0]->category_id ?? 0;
                ...
                //topproducts of customer which is loggedin
                // $topProductIds = DB::table('order_details')
                //     ->select('product_id', DB::raw('COUNT(*) as total'))
                //     ->groupBy('product_id')
                //     ->orderByDesc('total')
        ...
        $newlyAddedProducts = $this->product->active()->with([
            'wish_list' => function ($query) {
                return $query->where('customer_id', Auth::guard('customer')->user()->id ?? 0);
            },
            'compare_list' => function ($query) {
        ...
        $topSoldProducts = DB::table('order_details')
        ...
        $topProducts = $this->product->with([
            ...
            ->join('product_for_you', 'products.id', '=', 'product_for_you.product_id')
            ->orderBy('product_for_you.created_at', 'desc')
            ->limit(10)
            ->get();
    ...
    public function checkCitySession()
    {
        $hasLocation = session()->has('latitude') && session()->has('longitude');
