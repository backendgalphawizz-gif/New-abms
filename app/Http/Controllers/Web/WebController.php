...
class WebController extends Controller
{
    ...
    public function all_categories()
    {
        ...
        // dd($categories);
        return view(VIEW_FILE_NAMES['categories'], compact('categories'));
    ...
    public function all_brands(Request $request)
    {
        if (! Brand::tableAvailable()) {
            $brands = new LengthAwarePaginator([], 0, 24, (int) $request->get('page', 1), [
                'path' => $request->url(),
                'query' => $request->query(),
            ]);
    ...
    public function checkout_details(Request $request)
    {
        ...
        foreach($cart_group_ids as $group_id) {
            ...
            if($physical_product) {
                foreach ($carts as $cart) {
                    if ($shippingMethod == 'inhouse_shipping1') {
    ...
    {   
        ...
        $amount = CartManager::cart_grand_total() - $coupon_discount;
        if($request->has('group_id') && $request->input('group_id') != "") {
            $amount = CartManager::cart_grand_total($request->input('group_id')) - $coupon_discount;
        }
        $inr=Currency::where(['symbol'=>'₹'])->first();
        $usd=Currency::where(['code'=>'USD'])->first();
        $myr=Currency::where(['code'=>'MYR'])->first();
    ...
    public function discounted_products(Request $request)
    {
        ...
        if ($request['data_from'] == 'brand') {
            if (Brand::tableAvailable()) {
                ...
                $data['brand_name'] = $brand ? $brand->name : ($request['name'] ?? '');
            } else {
    ...
    public function viewWishlist(Request $request)
    {
        ...
        $brand_setting = Helpers::get_business_settings('product_brand') ?? '0';
        ...
        return view(VIEW_FILE_NAMES['account_wishlist'], compact('wishlists', 'brand_setting','final_category'));
    }
    ...
    {
        if (auth('customer')->check()) {
            $group_id = session()->get('buy_now');
            return view(VIEW_FILE_NAMES['buy_list'])->with('group_id', $group_id);
        }
        // Toastr::info(translate('no_items_in_basket'));
        return redirect()->back()->with('error', translate('no_items_in_basket'));
    }

    public function send_notification(Request $request)
    {
        $fcm_token = $request->input('fcm_key');
        $data = [
        ...
        $resp = Helpers::send_push_notif_to_device($fcm_token, $data);
