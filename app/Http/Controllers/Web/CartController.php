...
class CartController extends Controller
{
    ...
    public function variant_price(Request $request)
    {
        ...
        return [
            ...
            'static_variant_price' => \App\CPU\Helpers::currency_converter($price),
            'unit_price' => \App\CPU\Helpers::currency_converter($unit_price),
    ...
    public function save_later(Request $request)
    {
        ...
        $wishlist = Savelist::where('customer_id', auth('customer')->id())->where('product_id', $request->productid)->first();
        ...
        if (empty($wishlist)) {
            ...
            Cart::where(['customer_id' => auth('customer')->id(), 'product_id' => $request->productid])->delete();

            return response()->json(['status' => true, 'message' => translate('successfully added to save later!'), 'data' => []], 200);
        }

        return response()->json(['status' => false, 'message' => translate('Already added in save later'), 'data' => []], 200);
    }

    public function remove_save_later(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'productid' => 'required',
        ]);
        ...
        $wishlist = Savelist::where('customer_id', auth('customer')->id())->where('product_id', $request->productid)->delete();

        return response()->json(['status' => true, 'message' => translate('Removed from saved items'), 'data' => []], 200);
    }
    ...
    public function removeFromCart(Request $request)
    {
        ...
        return response()->json(['data' => view(VIEW_FILE_NAMES['products_cart_details_partials'])->render(), 'message'=>translate('Item_has_been_removed_from_cart'), 'dt' => ['id' => $request->key, 'customer_id' => auth('customer')->id()]]);
    }
