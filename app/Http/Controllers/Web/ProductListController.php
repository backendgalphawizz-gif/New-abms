...
class ProductListController extends Controller
{
    ...
    public function products(Request $request)
    {
        ...
        return match ($theme_name){
            ...
            'alpha_ecom' => self::alpha_ecom($request),
    ...
    public function alpha_ecom($request){
        $request['sort_by'] == null ? $request['sort_by'] == 'latest' : $request['sort_by'];
        ...
        if (!$request->has('data_from') || $request['data_from'] == 'product_for_you') {
            $query = $porduct_data;
        }
        ...
        if ($request['data_from'] == 'featured_deal') {
            $featured_deal_id = FlashDeal::where(['status'=>1])->where(['deal_type'=>'special-offers', 'id' => $request->input('id')])->pluck('id')->first();
            $featured_deal_product_ids = FlashDealProduct::where('flash_deal_id',$featured_deal_id)->pluck('product_id')->toArray();
        ...
        if ($request['data_from'] == 'vendors') {
            $query = Product::with(['reviews'])->active()->where(['user_id' => $request['id'], 'added_by' => 'seller']);
        }
        ...
        if ($request->ajax()) {
            return response()->json([
                'total_product'=>$products->total(),
                'view' => view(VIEW_FILE_NAMES['products__ajax_partials'], compact('products', 'final_category', 'product_ids'))->render(),
            ], 200);
        }
        if ($request['data_from'] == 'category') {
            $data['brand_name'] = Category::find((int)$request['id'])->name;
        ...
        $query1 = null;
        if ($request['data_from'] == 'product_for_you') {
        ...
        return view(VIEW_FILE_NAMES['products_view_page'], compact('products','final_category', 'data', 'ratings', 'product_ids'));
    }
    ...
    public function theme_fashion(Request $request)
    {
        ...
        if ($request['data_from'] == 'vendors') {
            $query = Product::with(['reviews'])->active()->where(['seller_id' => $request['id'], 'is_seller' => 1]);
        }
