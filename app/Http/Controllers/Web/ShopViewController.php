...
class ShopViewController extends Controller
{
    ...
    public function seller_shop(Request $request, $id)
    {
        ...
        return match ($theme_name){
            ...
            'alpha_ecom' => self::alpha_ecom($request, $id),
    ...
    public function alpha_ecom($request, $id){
        $business_mode=Helpers::get_business_settings('business_mode');
        ...
        $categories = [];
        foreach ($category_ids as $category_id) {
            $category = Category::has('product')->with(['childes.childes'])->where('position', 0)->find($category_id);
            if ($category != null) {
                array_push($categories, $category);
            }
        }
        ...
        $brands = Brand::tableAvailable()
            ? Brand::active()->whereIn('id', $brand_info)->withCount('brandProducts')->latest()->get()
            : collect();
    ...
    public function ajax_fashion_products(Request $request)
    {
        ...
        {
            $brands = Brand::tableAvailable()
                ? Brand::whereIn('id', $request->brand)->select('id', 'name')->get()
                : collect();
        }
    ...
    public function theme_all_purpose($request, $id){
        ...
        if($request['tab'] == 'all_product'){
            ...
            foreach ($products as $product) {
                ...
                $brands = Brand::tableAvailable()
                    ? Brand::active()->whereIn('id', $brand_info)
