...
class DeliveryManController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $data['delivery_man'] = $delivery_men->map(function ($man) {
            if (!empty($man->identity_image) && is_string($man->identity_image)) {
                $man->identity_image = json_decode($man->identity_image, true);
            }
            return $man;
        })->toArray();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('identity_image')) {
            ...
            if (is_array($files)) {
                ...
                array_push($id_img_names, ImageManager::upload('delivery-man/', 'png', $files));
            }
        }
        ...
        $dm->identity_type        = 'aadharcard'; //$request->identity_type;
        $dm->identity_image       = $identity_image;
        $dm->image                = ($request->file('image')) ? ImageManager::upload('delivery-man/', 'png', $request->file('image')) : null;
        $dm->license_image        = ($request->file('license_image')) ? ImageManager::upload('delivery-man/', 'png', $request->file('license_image')) : null;
        $dm->insurence_image      = $request->file('insurence_image') ? ImageManager::upload('delivery-man/', 'png', $request->file('insurence_image')) : null;
        $dm->rc_image             = $request->file('rc_image') ? ImageManager::upload('delivery-man/', 'png', $request->file('rc_image')) : null;
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$delivery_man) {
            return response()->json(['message' => translate('Delivery man not found!')], 404);
        }


        // Handle identity images (multiple support)
        if ($request->hasFile('identity_image')) {
            // Delete old images
            $old_images = json_decode($delivery_man->identity_image, true);
            ...
            if (is_array($files)) {
                foreach ($files as $img) {
                    $img_keeper[] = ImageManager::upload('delivery-man/', 'png', $img);
                }
            } else {
                $img_keeper[] = ImageManager::upload('delivery-man/', 'png', $files);
            }
        ...
        $delivery_man->email     = $request->email ?? $delivery_man->email;
        ...
        $delivery_man->identity_type   = 'aadharcard'; // $request->identity_type;
        $delivery_man->identity_image  = $identity_image;

        $delivery_man->image = $request->hasFile('image')
            ? ImageManager::update('delivery-man/', $delivery_man->image, 'png', $request->file('image'))
            : $delivery_man->image;

        $delivery_man->license_image = $request->hasFile('license_image')
            ? ImageManager::update('delivery-man/', $delivery_man->license_image, 'png', $request->file('license_image'))
            : $delivery_man->license_image;

        $delivery_man->insurence_image = $request->hasFile('insurence_image')
            ? ImageManager::update('delivery-man/', $delivery_man->insurence_image, 'png', $request->file('insurence_image'))
            : $delivery_man->insurence_image;

        $delivery_man->rc_image = $request->hasFile('rc_image')
            ? ImageManager::update('delivery-man/', $delivery_man->rc_image, 'png', $request->file('rc_image'))
            : $delivery_man->rc_image;

        // Password update is commented out but can be added back with validation

        $delivery_man->save();

        return response()->json(['message' => translate('Delivery man updated successfully!')], 200);
    }
    ...
    //     $delivery_man->license_image   = ($request->file('license_image')) ? ImageManager::update('delivery-man/', $delivery_man->license_image, 'png', $request->file('license_image')) : $delivery_man->license_image;
    //     $delivery_man->insurence_image = $request->file('insurence_image') ? ImageManager::update('delivery-man/', $delivery_man->insurence_image, 'png', $request->file('insurence_image')) : $delivery_man->insurence_image;
    //     $delivery_man->rc_image        = $request->file('rc_image') ? ImageManager::update('delivery-man/', $delivery_man->rc_image, 'png', $request->file('rc_image')) : $delivery_man->rc_image;
    ...
    public function details(Request $request, $id)
    {
        ...
        if ($delivery_man) {
            // Decode identity_image JSON string if it's not empty and is a string
            if (!empty($delivery_man->identity_image) && is_string($delivery_man->identity_image)) {
                $delivery_man->identity_image = json_decode($delivery_man->identity_image, true);
            }
        }
