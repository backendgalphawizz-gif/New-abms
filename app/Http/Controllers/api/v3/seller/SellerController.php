...
class SellerController extends Controller
{
    ...
    public function get_earning_statitics(Request $request)
    {
        ...
        // ---------------- Get MAIN requested type (same as before) ----------------
        if ($dateType == 'yearEarn') {
            ...
            $seller_earnings = OrderTransaction::where([
                ...
                DB::raw('SUM(seller_amount) as sums'),
                DB::raw('MONTH(created_at) as month')
            )->whereBetween('created_at', [$from, $to])
                ->groupBy('month')
                ->pluck('sums', 'month')->toArray();

            for ($i = 1; $i <= $months; $i++) {
                $seller_data_final[] = $seller_earnings[$i] ?? 0;
            }
            ...
            $commission_earnings = OrderTransaction::where([
                ...
                DB::raw('SUM(admin_commission) as sums'),
                DB::raw('MONTH(created_at) as month')
            )->whereBetween('created_at', [$from, $to])
                ->groupBy('month')
                ->pluck('sums', 'month')->toArray();
            ...
            for ($i = 1; $i <= $months; $i++) {
                $commission_data_final[] = $commission_earnings[$i] ?? 0;
            }
        ...
        } elseif ($dateType == 'dayEarn') {
            $from = Carbon::now()->startOfDay();
            $to = Carbon::now()->endOfDay();

            $seller_total = OrderTransaction::where([
                'seller_is' => 'seller',
                'seller_id' => $seller->id,
                'status' => 'disburse'
            ])->whereBetween('created_at', [$from, $to])->sum('seller_amount');

            $commission_total = OrderTransaction::where([
                'seller_is' => 'seller',
                'seller_id' => $seller->id,
                'status' => 'disburse'
            ])->whereBetween('created_at', [$from, $to])->sum('admin_commission');

            $seller_data_final = [$seller_total];
            $commission_data_final = [$commission_total];
        }

        // ✅ Total Sales = seller + commission
        $total_sales = array_sum($seller_data_final) + array_sum($commission_data_final);

        // ---------------- Always calculate ALL 4 TOTALS ----------------
        $day_total = OrderTransaction::where([
            'seller_is' => 'seller',
            'seller_id' => $seller->id,
            'status' => 'disburse'
        ])->whereBetween('created_at', [Carbon::now()->startOfDay(), Carbon::now()->endOfDay()])
            ->select(DB::raw('SUM(seller_amount + admin_commission) as total'))->value('total') ?? 0;
        ...
        // ✅ Response (same + extra fields)
        return response()->json([
            'seller_earn'     => $seller_data_final,
            'commission_earn' => $commission_data_final,
            'total_sales'     => $total_sales,
            'day_earn'        => $day_total,
            'week_earn'       => $week_total,
            'month_earn'      => $month_total,
            'year_earn'       => $year_total,
        ], 200);
    }
    ...
    public function shop_status_update(Request $request)
    {
        $seller = $request->seller;

        if (! $seller) {
            return response()->json(['message' => translate('invalid_seller')], 403);
        }
        $seller->shop_status = $request->shop_status;
        $seller->save();
