...
class ChatController extends Controller
{
    ...
    public function get_message(Request $request, $type, $id)
    {
        ...
        $query = Chatting::select('admin_id', 'user_id', 'message','sent_by_delivery_man', 'sent_by_admin', 'chattings.created_at')->where(['delivery_man_id' => $delivery_man['id'], $id_param => $id])->latest();
        $lists = $query->get();
        if (!empty($lists)) {
            ...
            foreach ($lists as $list) {
                ...
                $list->user_id = strval($list->user_id ?? 0);
                $list->sent_by_admin = strval($list->sent_by_admin ?? 0);
                $list->sent_by_delivery_man = strval($list->sent_by_delivery_man ?? 0);
            }
            // $message = $lists->paginate($request->limit, ['*'], 'page', $request->offset);

            // $lists->where($sent_by, 1)->update(['seen_by_delivery_man' => 1]);

            $data = array();
            $data['total_size'] = $lists->count();
            $data['limit'] = $request->limit;
            $data['offset'] = $request->offset;
            $data['message'] = $lists;
            return response()->json($data, 200);
    ...
    public function send_message(Request $request, $type)
    {
        ...
        if ($type == 'seller') {
            ...
            return response()->json(['status' => false, 'message' => translate('Invalid Chatting Type!'), 'time' => now()], 403);
        }
        ...
        if ($chatting->save()) {
            ...
            return response()->json(['status' => true, 'message' => $request->message, 'time' => now()], 200);
        } else {
            return response()->json(['status' => false, 'message' => translate('Message sending failed'), 'time' => now()], 403);
        }
    }
}
