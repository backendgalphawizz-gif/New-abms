...
class WithdrawController extends Controller
{
    public function withdraw_request(Request $request)
    {
        ...
        if(BackEndHelper::usd_to_currency($withdrawable_balance) < $request['amount']) {
            return response()->json(['status'=> false, 'message'=>translate('withdraw_request_amount_can_not_be_more_than_withdrawable_balance')], 403);
        }
        ...
        return response()->json(['status'=> false, 'message'=>translate('Invalid withdraw request')], 403);
    }
    ...
    public function withdraw_list_by_approved(Request $request)
    {
        ...
        $withdraw = WithdrawRequest::where(['delivery_man_id'=> $delivery_man->id]);
        ...
        $items = $withdraws->items();
        foreach($items as $item) {
            $item->amount = Helpers::set_symbol(BackEndHelper::usd_to_currency($item->amount));
            $item->admin_id = $item->admin_id ?? 0;
            $item->seller_id = $item->seller_id ?? 0;
            $item->withdrawal_method_id = $item->withdrawal_method_id ?? 0;
            $item->withdrawal_method_fields = $item->withdrawal_method_fields ?? [];
            $item->transaction_note = $item->transaction_note ?? "";
            $item->approved = $item->approved == 0 ? "Pending" : "Approved";
        }
