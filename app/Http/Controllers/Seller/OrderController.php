...
class OrderController extends Controller
{
    ...
    public function list(Request $request, $status)
    {
                ...
                $q->where(function ($qq) use ($key) {
                    foreach ($key as $value) {
                        $qq->where('id', 'like', "%{$value}%")
                            ...
                            ->orWhere('shipping_address_data->address1', 'like', "%{$value}%")
                            ->orWhere('shipping_address_data->area', 'like', "%{$value}%");
                    }
                });
            })
        ...
        $shipped_query = Order::where(['seller_is' => 'seller', 'order_status' => 'shipped', 'seller_id' => $sellerId]);
        $shipped = $this->common_query_status_count($shipped_query, $request);
    ...
    public function generate_invoice($id)
    {
        ...
        // return view('seller-views.order.invoice',compact('order', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo','taxDetails','taxAmountInWords','orderAmountInWords'));
        $mpdf_view = \View::make('seller-views.order.invoice', compact('order', 'taxDetails', 'qrCode', 'seller',  'company_phone', 'company_email', 'company_name', 'company_web_logo'));
