...
class DashboardController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        $data['drivers'] = DeliveryMan::where('seller_id', auth('seller')->id())->count();
        $data['total_order'] = Order::where('seller_id', auth('seller')->id())->count();
        $data['total_products'] = Product::where('added_by', 'seller')
        ->where('user_id', auth('seller')->id())
        ->count();
        $data['total_sale'] = Order::where('seller_id', auth('seller')->id())
        ->where('payment_status', 'paid')
        ->where('order_status', 'delivered')
        ->sum('order_amount');
        $data['today_sale'] = Order::where('seller_id', auth('seller')->id())
        ->whereBetween('created_at', [Carbon::now()->startOfDay(), Carbon::now()->endOfDay()])
        ->where('payment_status', 'paid')
        ->where('order_status', 'delivered')
        ->sum('order_amount');
        $data['today_orders'] = Order::where('seller_id', auth('seller')->id())
        ->whereBetween('created_at', [Carbon::now()->startOfDay(), Carbon::now()->endOfDay()])
        ->count();
     ...
     $totalSale = Order::select([DB::raw('SUM(order_amount) as total_amount')])->where(['payment_status' => 'paid', 'order_status' => 'delivered', 'seller_id' => auth('seller')->id(), 'seller_is' => 'seller'])->first();

     $response = [
         'status' => true,
         'message' => 'Seller Dashboar',
         'data' => [
             'total_sale' => Helpers::set_symbol(BackEndHelper::usd_to_currency($totalSale->total_amount)),
             'sold_out' => strval(Order::where(['payment_status' => 'paid', 'order_status' => 'delivered', 'seller_id' => auth('seller')->id(), 'seller_is'=>'seller'])->count() ?? 0),
             'total_product' => strval(Product::where(['user_id' => auth('seller')->id(), 'added_by'=>'seller'])->count() ?? 0),
             'total_orders' => strval(Order::where(['seller_id' => auth('seller')->id(), 'seller_is'=>'seller'])->count() ?? 0),
             'total_customers' => strval(User::count() ?? 0),
             'stock_management' => strval(Product::where('current_stock', '<', 5)->where(['user_id' => auth('seller')->id(), 'added_by'=>'seller'])->count() ?? 0),
             'total_delivery' => strval(Order::where(['payment_status' => 'paid', 'order_status' => 'out_for_delivery', 'seller_id' => auth('seller')->id(), 'seller_is'=>'seller'])->count()),
        ...
        /**
         * End Customized code for Dashboard
         */

        return view('seller-views.system.dashboard', compact('data', 'seller_data', 'commission_data', 'withdrawal_methods', 'response'));
    }
    ...
    public function order_stats_data()
    {
        ...
        $processing = Order::where(['seller_is' => 'seller'])->where(['seller_id' => auth('seller')->id()])->where(['order_status' => 'processing'])
            ...
            ->count();
        $shipped = Order::where(['seller_is' => 'seller'])->where(['seller_id' => auth('seller')->id()])->where(['order_status' => 'shipped'])
            ->when($today, function ($query) {
                return $query->whereDate('created_at', Carbon::today());
            })
            ->when($this_month, function ($query) {
