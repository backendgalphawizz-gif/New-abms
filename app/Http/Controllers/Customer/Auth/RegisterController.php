...
class RegisterController extends Controller
{
    ...
    /**
     * Phone/email verification rows use this key; null email must fall back to phone.
     */
    protected static function verificationIdentifier(User $user): string
    {
        return (string) ($user->email ?: $user->phone);
    }
    ...
    public function submit(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'phone.required' => translate('Please enter your phone number'),
            'phone.regex' => translate('Please enter your phone number'),
        ]);
    ...
    public static function varificaton_check($id)
    {
        ...
        DB::table('phone_or_email_verifications')->insert([
            'phone_or_email' => self::verificationIdentifier($user),
            'token' => $token,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        ...
        if ($email_verification && !$user->is_email_verified && $user->email) {
    ...
    public static function check($id)
    {
        ...
        $token = PhoneOrEmailVerification::where('phone_or_email', '=', self::verificationIdentifier($user))->first();
        if($token){
    ...
    public static function verify(Request $request)
    {
        ...
        $verify = PhoneOrEmailVerification::where(['phone_or_email' => self::verificationIdentifier($user), 'token' => $request['token']])->first();
        ...
        if (isset($verify)) {
            ...
            $verification = PhoneOrEmailVerification::where(['phone_or_email' => self::verificationIdentifier($user)])->first();
    ...
    public static function login_process($user, $email, $password)
    {
        if (auth('customer')->attempt(['email' => $email, 'password' => $password], true)) {
            session()->put('wish_list', Wishlist::productIdsForCustomerId((int) $user->id));
    ...
    public static function resend_otp(Request $request)
    {
        ...
        if($time_differance==0){
            if($token){
                ...
                $new_token->phone_or_email = self::verificationIdentifier($user);
            ...
            if ($email_verification && !$user->is_email_verified) {
                ...
                if ($email_services_smtp['status'] == 1 && $user->email) {
