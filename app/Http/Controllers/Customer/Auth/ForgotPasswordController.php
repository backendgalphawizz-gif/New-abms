...
class ForgotPasswordController extends Controller
{
    ...
    public function reset_password_request(Request $request)
    {
        ...
        if ($verification_by == 'email') {
            ...
            $customer = User::where('phone', 'like', "%{$request['identity']}%")->orWhere('email', 'like', "%{$request['identity']}%")->first();
            if (isset($customer)) {
                if(isset($password_verification_data) &&  Carbon::parse($password_verification_data->created_at)->diffInSeconds() < $otp_interval_time){
                    $time= $otp_interval_time - Carbon::parse($password_verification_data->created_at)->diffInSeconds();
                    ...
                    if($reset_data){
                        ...
                        $reset_data = new PasswordReset();
                        $reset_data->identity = $customer['phone'] ?? $customer['email'];
                        $reset_data->token = $token;
                        $reset_data->user_type = 'customer';
                        $reset_data->created_at = now();
                        $reset_data->updated_at = now();
                    ...
                    return redirect()->route('customer.auth.otp-verification', ['identity'=>$customer->phone, 'otp' => $token]);
                }
            }
        }
    ...
    public function reset_password_submit(Request $request)
    {
        ...
        if ($validator->fails()) {
            $response = ['status' => false, 'message' => 'Password and confirm password not matched'];
        ...
        if (isset($data)) {
            ...
            $user = User::where('email', 'like', "%{$data->identity}%")
                ->orWhere('phone', 'like', "%{$data->identity}%")
                ->first();
            if(Hash::check($request['password'], $user->password)) {
                $response = ['status' => false, 'message' => 'Password should not same as old one.'];
                return response()->json($response, 200);
            }
