...
class TransactionReportController extends Controller
{
    public function order_transaction_list(Request $request)
    {
        ...
        $payment_data['cash_payment'] = ApplicationPaymentDetail::selectRaw("
                        ...
                        AS total_earning
                    ")->value('total_earning');
        $payment_data['offline_payment'] = ApplicationPaymentDetail::selectRaw("
        ...
        $payment_data['total_payment'] = $payment_data['cash_payment'] + $payment_data['offline_payment'];  
        
        // $query_param = ['search' => $search, 'status' => $status, 'customer_id' => $customer_id, 'date_type' => $date_type, 'from' => $from, 'to' => $to];
        $fees = ApplicationPaymentDetail::with('application')->paginate(Helpers::pagination_limit())->appends($query_param);

        return view('admin-views.transaction.order-list', compact('customers', 'sellers', 'transactions', 'search', 'status',
            'from', 'to', 'customer_id', 'seller_id', 'payment_status', 'order_data', 'date_type', 'payment_data', 'order_transaction_chart','fees'));
    }
