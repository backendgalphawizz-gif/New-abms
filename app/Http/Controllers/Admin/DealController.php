...
class DealController extends Controller
{
    ...
    public function flash_submit(Request $request)
    {
        $flash_deal_id = DB::table('flash_deals')->insertGetId([
            ...
            'deal_type' => $request['deal_type'] == 'flash_deal' ? 'flash_deal' : 'special-offers',
            'status' => 0,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    public function feature_status(Request $request)
    {
        ...
        // FlashDeal::where(['status' => 1])->where(['deal_type' => 'special-offers'])->update(['status' => 0]);
        FlashDeal::where(['id' => $request['id']])->update([
            'status' => $request['status'],
        ]);
        return response()->json([
    ...
    public function feature_index(Request $request)
    {
        ...
        if ($request->has('search')) {
            $key = explode(' ', $request['search']);
            $flash_deals = FlashDeal::where('deal_type', 'special-offers')
                ->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->Where('title', 'like', "%{$value}%");
                    }
    ...
    foreach ($request['product_id'] as $product_id) {
        ...
        $flash_deal_products = FlashDealProduct::where('flash_deal_id', $deal_id)
