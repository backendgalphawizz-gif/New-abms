...
class SellerController extends Controller
{
    public function index(Request $request)
    {
        ...
        $sellers = Seller::with(['orders', 'product', 'shop.areas'])
            ->when($search, function($query) use($search){
                ...
                foreach ($key as $value) {
                    $q->orWhere('f_name', 'like', "%{$value}%")
                        ...
                        ->orWhereHas('shop.areas', function($q2) use ($value) {
            ...
            ->when($areas, function($query) use ($areas) {
                $query->whereHas('shop.areas', function($q) use ($areas) {
                    $q->whereIn('id', $areas);
                });
            })
        ...
        return view('admin-views.seller.index', compact('sellers', 'search','areas', 'current_date'));
    }

    public function view(Request $request, $id, $tab = null)
    {
        $seller = Seller::with('shop.areas','shop.cities','shop.states')->where('id',$id)->first();
        if(!isset($seller))
        {
        ...
        if ($tab == 'order') {
            ...
            $subscriptions = SellerSubscription::where('seller_id', $id)->where('type', 2);
            ...
            {
                $key = explode(' ', $request['search']);
                $subscriptions = $subscriptions->where(function ($q) use ($key) {
                    foreach ($key as $value) {
            ...
            $subscriptions = $subscriptions->latest()->paginate(Helpers::pagination_limit())->appends($query_param);

            return view('admin-views.seller.view.subscription', compact('seller', 'status', 'search', 'subscriptions'));
        } else if ($tab == 'requested-categories') {
            $transactions = DB::table('seller_request_for_categories')->where('seller_id',$seller->id);
                ...
                $transactions = $transactions->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->Where('category_name', 'like', "%{$value}%")
                            ->orWhere('sub_category_name', 'like', "%{$value}%")
                            ->orWhere('sub_sub_category_name', 'like', "%{$value}%");
                    }
                });
                $query_param = ['search' => $request['search']];
            }else{
            ...
            return view('admin-views.seller.view.request-categories', compact('seller', 'transactions','search','status'));
        } else if ($tab == 'supplier') {
            $transactions = Supplier::where('seller_id', $seller->id);
            ...
            return view('admin-views.seller.view.supplier', compact('seller', 'transactions','search','status'));
        }
    ...
    public function withdraw_list_export_excel(Request $request){
        ...
        $withdraw_requests->map(function ($query) {
            //company info
            $query->name = isset($query->seller) ? $query->seller->f_name . ' ' . $query->seller->l_name : '';
        ...
        foreach ($withdraw_requests as $key=>$item) {
            ...
            unset($item['withdraw_method_name']);
        }
    ...
    public function add_seller()
    {
        ...
        return view('admin-views.seller.add-new-seller', compact('states'));
    }

    public function sellerLogin(Request $request, $seller_id) {
        $se = Seller::find($seller_id);
    ...
    public function updateCategoryRequest(Request $request) {
        ...
        if(DB::table('seller_request_for_categories')->where('id', $id)->update(['status' => $status])) {
            return response()->json(['status' => true, 'message' => 'Request Category Status updated success']);
        } 
