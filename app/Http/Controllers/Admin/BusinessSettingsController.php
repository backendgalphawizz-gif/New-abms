...
class BusinessSettingsController extends Controller
{
    ...
    public function vendor_page($page)
    {
        ...
        if (in_array($page, $pages)) {
            $data = BusinessSetting::where('type', $page)->first();
            return view('admin-views.business-settings.vendor-page', compact('page', 'data'));
        }
    ...
    public function vendor_page_update(Request $request, $page)
    {
        $request->validate([
            'value' => 'required',
        ]);
    ...
    public function driver_page($page)
    {
        $pages = array(
            'driver-terms-policy',
            'driver-privacy-policy',
            'driver-refund-policy',
            'driver-return-policy',
            'driver-cancellation-policy',
            'driver-shipping-policy',
            'driver-security-policy-policy',
            'driver-payment-policy',
            'driver-condition-of-use-policy',
            'driver-security-information',
            'driver-product-listing-policy',
            'driver-commission-and-fees-policy',
            'driver-order-fulfillment-policy',
            'driver-communication-policy',
            'driver-delivery-guidelines',
            'driver-package-handling-policy',
            'driver-customer-interaction-policy'
        );

        if (in_array($page, $pages)) {
            $data = BusinessSetting::where('type', $page)->first();
            return view('admin-views.business-settings.driver-page', compact('page', 'data'));
        }
    ...
    public function driver_page_update(Request $request, $page)
    {
        $request->validate([
            'value' => 'required',
        ]);
    ...
    public function companyInfo()
    {
        ...
        $app_marque_text = BusinessSetting::where('type', 'app_marque_text')->first();
    ...
    public function productSettings()
    {
        ...
        return view('admin-views.business-settings.product-settings', compact('company_name', 'company_email', 'company_phone','app_marque_text', 'digital_product', 'brand'));
    }
    ...
    public function updateInfo(Request $request)
    {
        ...
        DB::table('business_settings')->updateOrInsert(['type' => 'app_marque_text'], [
            'value' => $request['app_marque_text']
        ]);
        ...
        DB::table('business_settings')->updateOrInsert(['type' => 'bank_holder_name'], [
            'value' => $request['bank_holder_name']
        ]);

        DB::table('business_settings')->updateOrInsert(['type' => 'bank_account_number'], [
            'value' => $request['bank_account_number']
        ]);

        DB::table('business_settings')->updateOrInsert(['type' => 'bank_ifsc'], [
            'value' => strtoupper($request['bank_ifsc']) // always uppercase
        ]);

        DB::table('business_settings')->updateOrInsert(['type' => 'bank_name'], [
            'value' => $request['bank_name']
        ]);

        DB::table('business_settings')->updateOrInsert(['type' => 'bank_branch_name'], [
            'value' => $request['bank_branch_name']
        ]);
        ...
        // Save app header colors
        $appColors = BusinessSetting::where('type', 'app_header_colors')->first();
        if ($appColors) {
            BusinessSetting::where('type', 'app_header_colors')->update([
                'value' => json_encode([
    ...
    public function updateCompanyappmarquetext(Request $data)
    {
        $validatedData = $data->validate([
            'app_marque_text' => 'required',
        ]);
        BusinessSetting::where('type', 'app_marque_text')->update(['value' => $data->app_marque_text]);
        Toastr::success('Company header marque text successfully!');
        return redirect()->back();
    }
    ...
    public function viewAppHome(Request $request)
    {

        $app_setting = BusinessSetting::where('type', 'app_home_page')->first();

        $settings = json_decode($app_setting->value, true);

        $products = Product::get();

        return view('admin-views.business-settings.social-login.app-home', compact('settings', 'products'));
    }
    ...
    public function updateAppHome(Request $request)
    {
        ...
        $setting = json_decode($app_setting->value, true);

        $values['frame_one'] = [
            'label_1' => $request->input('label_1'),
            'label_2' => $request->input('label_2'),
            'label_3' => $request->input('label_3')
        ];

        $values['top_deal_product'] = [
        ...
        $values['gift_section'] = [
            ...
            'gift_product_ids' => implode(',', $request->input('gift_product_ids'))
        ];

        $summer_sale_banner = $setting['summer_sale_banner']['image'] ?? "";
        $prime_time_banner = $setting['prime_time_banner']['image'] ?? "";

        if ($request->has('banner_image')) {
            $summer_sale_banner = ImageManager::upload('product/', 'png', $request->file('banner_image'));
        }

        if ($request->has('prime_image')) {
            $prime_time_banner = ImageManager::upload('product/', 'png', $request->file('prime_image'));
        }

        $values['summer_sale_banner'] = [
            'image' => $summer_sale_banner,
            'discount_percent' => $request->input('discount_percent')
        ];

        $values['prime_time_banner'] = [
            'image' => $prime_time_banner
        ];

        if (empty($app_setting)) {
            $app_setting = new BusinessSetting;
            $app_setting->type = 'app_home_page';
        }

        $app_setting->value = json_encode($values);
        $app_setting->save();

        Toastr::success('App Home Page Setting updated!');
        return back();
    }
