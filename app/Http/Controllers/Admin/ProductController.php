...
class ProductController extends BaseController
{
    public function add_new()
    {
        ...
        return view('admin-views.product.add-new', compact('cat', 'br', 'brand_setting', 'digital_product_setting', 'sellers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request['unit_price'] < $request['selling_price']) {
            $validator->after(function ($validator) {
                $validator->errors()->add(
                    'selling_price',
                    'Seller price should not be greater than MRP!'
                );
            });
        }
        ...
        $product = new Product();
        $product->user_id  = $request->user_id ?? auth('admin')->id();
        $product->added_by = $request->added_by ??   "admin";
        ...
        $product->is_bulk_pricing       = isset($request->is_bulk_pricing) ? 1 : 0;
        ...
        $product->pack_quantity            = $request->product_type == 'physical' ? $request->pack_quantity : null;
        ...
        $product->package_weight               = $request->product_type == 'physical' ? $request->package_weight : null;
        $product->per_pack_quantity               = $request->product_type == 'physical' ? $request->per_pack_quantity : null;
        ...
        $variantArray = [];
        if (isset($request['is_bulk_pricing'])) {
            foreach ($request['bulk_variant'] as $key => $value) {
                $variantArray[$key]['variant'] = $value['variant'];
                $variantArray[$key]['array_length'] = count($value['from']);
                $variantArray[$key]['from'] = $value['from'];
                $variantArray[$key]['to'] = $value['to'];
                $variantArray[$key]['bulk_price'] = $value['bulk_price'];
            }
        }

        // end tier
        $product->variation      = $request->product_type == 'physical' ? json_encode($variations) : json_encode([]);
        $product->bulk_pricing   = isset($request['is_bulk_pricing']) ? json_encode($variantArray) : json_encode([]);
        $product->unit_price     = Convert::usd($request->unit_price);
        $product->selling_price  = Convert::usd($request->selling_price);
        $product->purchase_price = 0; //Convert::usd($request->purchase_price);
        $product->tax            = $request->tax;
        $product->tax_type       = $request->tax_type;
        $product->tax_model      =  "include"; //$request->tax_model;
    ...
    //     $p = new Product();
    //     $p->user_id  = $request->user_id ?? auth('admin')->id();
    //     $p->added_by = $request->added_by ??   "admin";
    ...
    //     $p->net_weight                 = $request->product_type == 'physical' ? $request->net_weight : null;
    //     $p->per_unit_price                 = $request->product_type == 'physical' ? $request->per_unit_price : null;
    //     $p->package                 = $request->product_type == 'physical' ? $request->package : null;
    ...
    //     $p->free_delivery = $request->free_delivery ?? 0;
    //     $p->returnable = $request->returnable ?? 0;
    //     $p->is_warranty_policy = $request->is_warranty_policy ?? 0;
    //     $p->top_brand = $request->top_brand ?? 0;
    //     $p->is_verified = $request->is_verified ?? 0;
    //     $p->made_in = $request->made_in ?? "";
    //     $p->warranty = $request->warranty ?? "";
    //     $p->manufacturing_date = $request->manufacturing_date ?? date('Y-m-d');
    ...
    //     $p->discount_price    = BackEndHelper::currency_to_usd($request->unit_price - $dis);
    ...
    //             // if ($request->specification[$index] && $key != 'en') {
    //             //     array_push($data, array(
    //             //         'translationable_type' => 'App\Model\Product',
    //             //         'translationable_id' => $p->id,
    //             //         'locale' => $key,
    //             //         'key' => 'specification',
    //             //         'value' => $request->specification[$index],
    //             //     ));
    //             // }
    //         }
    //         Translation::insert($data);
    ...
    function list_paginate(Request $request)
    {
        ...
        // } else {
        // }
        $pro = Product::orderBy($orderbyName, $orderBy)->limit($request->length)->offset($request->start);
        ...
        foreach ($pro as $k => $p):
            ...
            $isFeatured = $p->featured == 1 ? 'checked' : '';
            $statusCheck = $p->status == 1 ? 'checked' : '';
            ...
            $dt[] = [
                ...
                ($p->category->name ?? ""),
                ($p->subcategory->name ?? ""),
                ($p->seller->shop->name ?? "Admin"),
                \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($p['purchase_price'])),
                \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($p['unit_price'])),
                "<label class='mx-auto switcher'>
                    <input class='switcher_input' type='checkbox'
                            onclick='featured_status('" . $p['id'] . "')' " . $isFeatured . ">
                    <span class='switcher_control'></span>
                </label>",

                "<label class='mx-auto switcher'>
                    <input type='checkbox' class='status switcher_input' id='" . $p['id'] . "' " . $statusCheck . ">
                    <span class='switcher_control'></span>
                </label>",
    ...
    public function update_price(Request $request, $id)
    {
        ...
        $type = $request['type'];
        $message = \App\CPU\translate('product_updated_successfully'); // default

        // mrp section
        if ($type == 'mrp') {
            $new_price = floatval($request['new_price']);
            $old_selling_price = floatval($old_selling_price);

            if ($old_selling_price > $new_price || $new_price == 0) {
                Toastr::warning(\App\CPU\translate('mrp_can_not_be_less_than_selling_price!'));
                return back();
            }

            $discount = (($new_price - $old_selling_price) / $new_price) * 100;
            $discount = number_format($discount, 2);

            $product->unit_price = Convert::usd($new_price);
            $product->discount = $discount;
            $product->discount_type = 'percent';

            $message = \App\CPU\translate('MRP updated successfully!');
        }

        // selling_price
        if ($type == 'selling_price') {
            $selling_price = floatval($request['selling_price']);
            $old_unit_price = floatval($old_unit_price);

            if ($old_unit_price < $selling_price || $selling_price == 0) {
                Toastr::warning(\App\CPU\translate('selling_price_can_not_be_grater_than_selling_price!'));
                return back();
            }

            $discount = (($old_unit_price - $selling_price) / $old_unit_price) * 100;
            $discount = number_format($discount, 2);
            ...
            $message = \App\CPU\translate('Selling price updated successfully!');
        }
        ...
        if ($type == 'discount') {
            ...
            if ($discount_value > 99) {
                Toastr::warning(\App\CPU\translate('discount_can_not_be_grater_than_99'));
                return back();
            }

            $selling_price = ($old_unit_price / 100) * $discount_value;
            $selling_price = $old_unit_price - $selling_price;

            $product->selling_price = Convert::usd($selling_price);
            $product->discount = $discount_value;
            $product->discount_type = 'percent';

            $message = \App\CPU\translate('Discount updated successfully!');
        }

        // quantity
        if ($type == 'quantity') {
            if ($request['quantity'] < 0) {
                Toastr::warning(\App\CPU\translate('Quantity can not be less than 0'));
                return back();
            }

            $product->current_stock = $request['quantity'];
            $message = \App\CPU\translate('Quantity updated successfully!');
        }
        ...
        if ($request->has('variations')) {
            ...
            foreach ($request->variations as $index => $varInput) {
                $type = $varInput['type'];
                $newPrice = $varInput['new_price'];

                $existing = collect($existingVariations)->firstWhere('type', $type);

                if ($existing) {
                    $updatedVariations[] = [
                        'type' => $type,
                        'price' => Convert::usd($newPrice),
                        'sku' => $existing['sku'] ?? '',
                        'per_pack_quantity' => $existing['per_pack_quantity'] ?? 0,
                        'qty' => $existing['qty'] ?? 0,
                    ];
                }
            }

            $product->variation = json_encode($updatedVariations);
            $message = \App\CPU\translate('Product variations updated successfully!');
        }
    ...
    //     // Update main product price if valid
    //     if ($request->filled('new_price') && $request['new_price'] > 0) {
    //         $product->unit_price = BackEndHelper::currency_to_usd($request['new_price']);
    //     } elseif ($request->filled('new_price') && $request['new_price'] <= 0) {
    //         Toastr::warning(\App\CPU\translate('product_price_can_not_be_less_than_0_!'));
    //         return back();
    //     }

    //     // Update variation prices if provided
    //     if ($request->has('variations')) {
    //         $existingVariations = json_decode($product->variation, true);
    //         $updatedVariations = [];
    ...
    //             // Find existing variation by type
    //             $existing = collect($existingVariations)->firstWhere('type', $type);

    //             if ($existing) {
    //                 $updatedVariations[] = [
    //                     'type' => $type,
    //                     'price' => BackEndHelper::currency_to_usd(($newPrice)),
    //                     'sku' => $existing['sku'] ?? '',
    //                     'qty' => $existing['qty'] ?? 0,
    //                 ];
    //             }
    ...
    //     Toastr::success(\App\CPU\translate('product_price_updated_successfully!'));
    //     return back();
    // }
    ...
    public function sku_combination(Request $request)
    {
        ...
        if ($product && $product->is_bulk_pricing && !empty($product->bulk_pricing)) {
            $bulkPricing = json_decode($product->bulk_pricing, true);
        }
        ...
        // dd($variations);
        return response()->json([
            'view' => view('admin-views.product.partials._sku_combinations', compact('combinations', 'unit_price', 'colors_active', 'product_name', 'variations'))->render(),
            'bulkview' => view('admin-views.product.partials._sku_bulk_combinations', compact('combinations', 'unit_price', 'colors_active', 'product_name', 'bulkPricing'))->render(),
        ]);
    }
    ...
    //         'bulkview' => view('admin-views.product.partials._sku_bulk_combinations', compact('combinations', 'unit_price', 'colors_active', 'product_name'))->render(),
    //     ]);
    // }
    ...
    public function edit($id)
    {
        ...
        return view('admin-views.product.edit', compact('sellers', 'categories', 'br', 'product', 'product_category', 'brand_setting', 'digital_product_setting'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $product->added_by = $request->added_by ??   "admin";
        $product->product_type          = $request->product_type;
        $product->is_bulk_pricing       = $request->has('is_bulk_pricing') ? 1 : 0;
        $product->category_ids          = json_encode($category);
        $product->category_id           = $request->category_id;
        $product->sub_category_id       = $request->sub_category_id;
        $product->sub_sub_category_id   = $request->sub_sub_category_id;
        ...
        if ($request->has('is_bulk_pricing') && isset($request->bulk_variant)) {
            foreach ($request->bulk_variant as $key => $value) {
                $variantArray[] = [
                    'variant'     => $value['variant'] ?? '',
                    'array_length' => isset($value['from']) ? count($value['from']) : 0,
                    'from'        => $value['from'] ?? [],
                    'to'          => $value['to'] ?? [],
        ...
        //combinations end
        $product->variation         = $request->product_type == 'physical' ? json_encode($variations) : json_encode([]);
        $product->bulk_pricing      = $product->is_bulk_pricing ? json_encode($variantArray) : json_encode([]);
        $product->unit_price        = Convert::usd($request->unit_price);
        $product->selling_price        = Convert::usd($request->selling_price);
    ...
    //     $product->free_delivery = $request->free_delivery ?? 0;
    //     $product->returnable = $request->returnable ?? 0;
    //     $product->is_warranty_policy = $request->is_warranty_policy ?? 0;
    //     $product->top_brand = $request->top_brand ?? 0;
    //     $product->is_verified = $request->is_verified ?? 0;
    //     $product->made_in = $request->made_in ?? "";
    //     $product->warranty = $request->warranty ?? "";
    //     $product->manufacturing_date = $request->manufacturing_date ?? date('Y-m-d');
    ...
    //     $product->discount_price    = BackEndHelper::currency_to_usd($request->unit_price - $dis);
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        $data = [];
        $col_key = ['name', 'category_id', 'sub_category_id', 'sub_sub_category_id', 'brand_id', 'unit', 'min_qty', 'net_weight','pack_quantity', 'youtube_video_url', 'unit_price', 'purchase_price', 'tax', 'discount', 'discount_type', 'current_stock', 'details', 'thumbnail'];
        ...
        foreach ($collections as $collection) {
            ...
            array_push($data, [
                ...
                'net_weight' => $collection['net_weight'],
                'pack_quantity' => $collection['pack_quantity'],
    ...
    public function product_for_you(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            // Search based on related Product's name
            $for_you = ProductForYou::whereHas('product', function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->where('name', 'like', "%{$value}%");
                }
            });
            ...
            $for_you = new ProductForYou();
        }

        $for_you = $for_you->latest()->paginate(Helpers::pagination_limit())->appends($query_param);

        $excluded_product_ids = ProductForYou::pluck('product_id')->toArray();

        $products = Product::where('status', 1)
            ->whereNotIn('id', $excluded_product_ids)
            ->orderBy('name', 'asc')
            ->get();

        return view('admin-views.product.product-for-you', compact('for_you', 'search', 'products'));
    }

    public function product_for_you_submit(Request $request)
    {
        $request->validate([
        ...
        foreach ($request['product_id'] as $product_id) {
            $exists = ProductForYou::where('product_id', $product_id)->exists();

            if (!$exists) {
                $for_you_id = DB::table('product_for_you')->insertGetId([
                    'product_id' => $product_id,
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);

                $inserted_ids[] = $for_you_id;
            }
        }

        Toastr::success('Product(s) added successfully!');

        return redirect()->route('admin.product.product-for-you');
    }
    ...
    // if ($request->has('search')) {
    //     $key = explode(' ', $request['search']);
    //     $for_you = ProductForYou::where(function ($q) use ($key) {
    //         foreach ($key as $value) {
    ...
    //     $for_you = ProductForYou::latest()->paginate(Helpers::pagination_limit());
    ...
    //     return redirect()->route('admin.product.product-for-you')
    //         ->with('for_you_id', $for_you_id)
    //         ->with('for_you', $for_you);
    // }


    public function product_for_you_delete(Request $request)
    {
        ProductForYou::destroy($request->id);
