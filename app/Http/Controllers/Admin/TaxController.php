...
class TaxController extends Controller
{
    public function add_new(Request $request)
    {
        ...
        $cou = Tax::when(isset($request['search']) && !empty($request['search']), function ($query) use ($search) {
                    $key = explode(' ', $search);
        ...
        return view('admin-views.tax.add-new', compact('cou', 'customers', 'sellers', 'search'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->percent > 100) {
            Toastr::error('The minimum tax percent exceeds');
            return redirect()->back();
        }

        $coupon = new Tax();
        $coupon->name = $request->title;
        $coupon->slug = strtolower(str_replace(' ', '-', $request->title));
    ...
    public function edit($id)
    {
        $sellers = Seller::with('shop')->approved()->get();
        $customers = User::where('id', '<>', '0')->get();
        $c = Tax::where(['added_by' => 'admin'])->find($id);
        if(!$c){
            Toastr::error('Invalid Tax!');
            return redirect()->route('admin.taxes.add-new');
        }
        return view('admin-views.tax.edit', compact('c', 'customers', 'sellers'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $coupon = Tax::where(['added_by' => 'admin'])->find($id);
    ...
    {
        $coupon = Tax::where(['added_by' => 'admin'])->find($request->id);
        $coupon->status = $request->status;
        $coupon->save();
    ...
    public function quick_view_details(Request $request)
    {
        ...
        return response()->json([
            'view' => view('admin-views.tax.details-quick-view', compact('coupon'))->render(),
        ]);
    }
