...
class OrderReportController extends Controller
{
    ...
    public function order_list_paginate(Request $request)
    {
        $seller_id = $request->seller_id ?? 'all';
        $date_type = $request->date_type ?? 'this_year';
        ...
        $orders = $orders->orderBy($orderbyName, $orderBy)->limit($request->length)->offset($request->start);
        ...
        $rows = [
            ...
            "recordsFiltered" =>  $filterCount ? count($dt) : Order::count(),
            "data" => $dt
        ];
    ...
    public function order_report_export_excel(Request $request){
        ...
        if($request->input('type') == 'csv') {
            return (new FastExcel($data))->download('order_report_list.csv');
        }
