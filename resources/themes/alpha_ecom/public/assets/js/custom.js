...
function currency_change(currency_code) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            swal.fire(`${data.success + data.name}`, '', 'success')
            location.reload();
        },
    });
}

function global_search(evt) {
    ...
    if(evt.keyCode == 40 || evt.keyCode == 38) {
        // Change focus to new div
        handleArrowKeys(evt.keyCode)
        // console.log('evt.target ', $(evt.target).parent().next(), $(evt.target).parent().next().find('ul li:first').focus());
        return false
    }
...
function handleArrowKeys(keyCode) {
    ...
    let $focusedItem = $("li.list-group-item:focus")

    // If no item is focused, focus the first item
    if ($focusedItem.length === 0) {
        $("li.list-group-item:first").focus()
        return;
    }

    // Handle arrow keys to navigate through the list
    if (keyCode === 40) {
        // Down arrow
        $focusedItem.next().focus()
    } else if (keyCode === 38) {
        // Up arrow
        $focusedItem.prev().focus()
    }
    console.log('focusedItem : ', $focusedItem)
}

function resetFocus() {
    // Reset focus to the first item when search results are updated
    $("li.list-group-item:first").focus()
}
...
function checkAddToCartValidity(form_id) {
    ...
    console.log(count, $("." + form_id + " input:radio:checked").length)
...
function buy_now(form_id, redirect_status, url = '') {
    if(addToCart(form_id, redirect_status, url)) {
...
function stock_check() {
    ...
    if (valueCurrent > maxValue) {
        ...
        swal.fire(`Sorry, stock limit exceeded`,'','warning')
...
function addToCart(form_id, redirect_to_checkout = false, url) {
    if (
        ...
        $.post({
            ...
            success: function (response) {
                if (response.status == 1) {
                    updateNavCart();
                    console.log(parseInt($('.cart-counter').text()))
        ...
        swal.fire($(`#` + form_id).data("outofstock"), '','warning');
        ...
        swal.fire($(`#` + form_id).data("errormessage"), '','info');
...
function updateCartQuantity(cart_id, product_id, action, event) {
    ...
    if (
        ...
        if(cart_quantity_of.val() < cart_quantity_of.data("min")){
            ...
            swal.fire(`'Minimum order quantity cannot be less than ${min_value}`,'','error')
    ...
    if (
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#add-to-cart-form .unit_price").html(data.unit_price);
                $("#add-to-cart-form .static_price").html(data.static_variant_price);
...
$("#contact_with_seller_form").on("submit", function (e) {
    ...
    $.ajax({
        ...
        success: function (respons) {
            swal.fire($("#contact_with_seller_form").data("success-message"),'','success')
            // toastr.success(
...
function coupon_copy(coupon) {
    ...
    swal.fire(`Successfully coupon copied`, '','success')
}
