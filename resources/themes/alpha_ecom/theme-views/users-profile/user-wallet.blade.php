                        ...
                        <div class="col-md-4">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="wallet-card pb-3 rounded-10  ov-hidden" data-bg-img="{{ theme_asset('assets/img/media/wallet-card.png') }}" style="--bg-color: var(--bs-primary-rgb);">
                                    ...
                                    <div class="wallet-card pb-3 rounded-10  ov-hidden">
                                        <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModal"><i class="fa fas-plus"></i> Add Wallet</button>
                                    </div>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('add-wallet') }}" id="add-wallet-amount">
                            @csrf
                            <div class="row">
                                <div class="col-lg-12">
                                    <label for="">Amount</label>
                                    <input type="number" name="amount" class="form-control">
                                    <input type="hidden" name="razorpay_id">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="addWalletAmount()">Add Money</button>
                    </div>
                    </div>
            </div>
        </div>
...
@push('script')
    <script>
        function addWalletAmount() {
            let amount = $('#add-wallet-amount').find('input[name=amount]').val()
            var rzp1 = openRazorPayToAddWallet(amount, new Date().getTime(), '')
            rzp1.open();
        }

        function openRazorPayToAddWallet(amount, order_id, plan_id) {
            ...
            var options = {
                ...
                "handler": function(response) {
                    $('input[name=razorpay_id]').val(response.razorpay_payment_id)
                    $('input[name=razorpay_signature]').val(response.razorpay_signature)
                    $('#add-wallet-amount').submit()
                },
                "prefill": {
                    "name": "Cartuko.com",
        ...
        $(document).on('submit','#add-wallet-amount', function(e) {
            e.preventDefault()
            if($('input[name=razorpay_id]').val() != "") {
                $.ajax({
                    type: "POST",
                    url: "{{ route('add-wallet') }}",
                    data: $(this).serialize(),
                    dataType: "json",
