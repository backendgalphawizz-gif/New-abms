<!-- Refund Modal -->
<div class="modal fade for-refund" id="refundModal{{$id}}" tabindex="-1" aria-labelledby="refundModalLabel" aria-hidden="true">
            ...
            <div class="modal-body px-sm-5">
                    <div class="d-flex flex-column flex-sm-row flex-wrap gap-4 justify-content-between mb-4">
                            ...
                            <div class="media-body d-flex gap-1 flex-column">
                                <h6 class="text-truncate" style="--width: 20ch">
                                    ...
                                    @if($order_details->variant)
                                        <small><b>{{translate('variant')}} </b>:{{$order_details->variant}} </small>
                        ...
                        <div class="d-flex flex-column gap-1 fs-12">
                            <span><b>{{('Qty')}}</b> : {{$order_details->qty}}</span>
                            <span><b>{{('Price')}}</b> : {{\App\CPU\Helpers::currency_converter($order_details->price)}}</span>
                            <span><b>{{('Discount')}}</b> : {{\App\CPU\Helpers::currency_converter($order_details->discount)}}</span>
                            <span><b>{{('Tax')}} </b>: {{\App\CPU\Helpers::currency_converter($order_details->tax)}}</span>
                        </div>
                        ...
                        <div class="d-flex flex-column gap-1 fs-12">
                            <span><b>{{translate('Subtotal')}}</b>: {{\App\CPU\Helpers::currency_converter($subtotal)}}</span>
                            <span><b>{{translate('Coupon_discount')}}</b>: {{\App\CPU\Helpers::currency_converter($coupon_discount)}}</span>
                            <span><b>{{translate('Total_refundable_amount')}}</b>:{{\App\CPU\Helpers::currency_converter($refund_amount)}}</span>
                        </div>
                    </div>
