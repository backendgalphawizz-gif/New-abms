...
<head>
    ...
    <!-- <link rel="stylesheet" href="{{ theme_asset('assets/css/Font-Awesome-Free-6.4.2-all.min.css')}}"> -->
    ...
    <link rel="stylesheet" href="{{ theme_asset('assets/css/price_range_style.css')}}" />
    ...
    <link rel="stylesheet" href="{{theme_asset('assets/css/jquery-ui.css')}}" media="all" />
    ...
    <link rel="stylesheet" href="{{theme_asset('assets/css/zoom-img.css')}}"/>
    <link rel="stylesheet" href="{{theme_asset('assets/css/lightbox.css')}}"/>

    <!--Toastr -->
    <link rel="stylesheet" href="{{theme_asset('assets/css/toastr.css')}}"/>
    <link rel="stylesheet" href="{{theme_asset('assets/css/intlTelInput.css')}}">
    <link rel="stylesheet" href="{{theme_asset('assets/css/zoom.css')}}">
    <link rel="stylesheet" href="{{theme_asset('assets/css/jquery.fancybox-thumbs.css')}}">

    
 
    <title>Prime Basket</title>
    <style>
        :root {
...
$bgColor: linear-gradient(130deg, #ece3ce, #90d5ff) fixed;
...
a {
  ...
  &:focus,
  &:active {
    color: scale-color($venus, $lightness: +35%);
  }
}
.social {
  stroke: currentColor;
  ...
  height: 24px;
  z-index: 10;
  animation: iconsLoad 4s ease both 2s;
...
<div id="preloader">
  ...
  <div class="loading_outer">
      <div class="loader-svg" style="flex-direction: column;">
       <img style="margin-bottom: 30px;" src="{{ theme_asset('assets/images/primeLogo.png') }}" alt="">
      <div class="main">
...
<script src="{{ theme_asset('assets/js/jquery.ui.caurousel.min.js') }}"></script>
<script src="{{ theme_asset('assets/js/lightbox.js') }}"></script>
<script src="{{ theme_asset('assets/js/zoom-img.js') }}"></script>
...
<script>
    ...
    $(document).on('click', '#refresh-login-modal', function () {
       
        $('#login-with-otp')[0].reset();  

   
        $('#submit-with-otp').find('input[name=phone]').prop('readonly', false);
        $('#submit-with-otp').find('input[name=phone]').val('');
        $('#submit-with-otp').find('input[name=otp]').val('');
        $('#submit-with-otp').find('span.otp-field').addClass('d-none');  
        ...
        swal.fire('Form has been reset', '', 'success');
    });
    ...
    $(document).on('keyup', '#login-with-otp input[name=phone]', function(evt){
        if(evt.keyCode == 13){
            $('#login-with-otp').submit()
        }
    })

    $(document).on('keyup', '#submit-with-otp input[name=phone], #submit-with-otp input[name=otp]', function(evt){
        if(evt.keyCode == 13){
            $('#submit-with-otp').submit()
        }
    })
...
var tables = document.querySelectorAll('.describtion table');

// Loop through each table and create a div as its parent
tables.forEach(function(table) {
    var div = document.createElement('div');
    div.classList.add("table-responsive");
    
    // Set styles for the div (adjust as needed)


    
    // Append the div as the parent of the table
