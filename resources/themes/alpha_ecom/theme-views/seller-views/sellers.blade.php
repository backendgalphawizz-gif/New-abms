...
@section('content')
    ...
    <!-- Main Content -->
    <main class="main-content d-flex flex-column gap-3 py-3 mb-30 customSection">
        <div class="container-fluid productcontent">
            ...
            <div class="card-body">
                <div class="row gy-2 align-items-center">
                    <div class="col-md-12">
                        <h3 class="mb-1">{{ translate('Distributer') }}</h3>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb fs-12 mb-0">
                                <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ translate('Home') }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">{{ translate('Vendor_List') }}</li>
                            </ol>
                        </nav>
                    </div>
                            ...
                            <div class="collapse navbar-collapse w-100" id="navbarSupportedContent">
                                <div class="accordion w-100" id="accordionExample">
                                    ...
                                    <div class="accordion-item">
                                        <h2 class="accordion-header" id="headingTwo{{$ckey}}">
                                            <button class="accordion-button collapsed black-text fw-600" type="button"
                                                data-bs-toggle="collapse" data-bs-target="#collapseTwo{{$ckey}}"
                                                aria-expanded="false" aria-controls="collapseTwo{{$ckey}}">
                                                {{ $category->name }}
                                            </button>
                                        </h2>
                                        <div id="collapseTwo{{$ckey}}" class="accordion-collapse collapse"
                                            aria-labelledby="headingTwo{{$ckey}}" data-bs-parent="#accordionExample">
                                            <div class="accordion-body ps-0 pt-0">
                                                <ul class="list-group">
                                                    @foreach($category->childes as $cc_key => $childCategory)
                ...
                <div class="col-lg-12 col-md-12 col-12">
                    <div class="row g-3">
                        ...
                        @php($product_ids=$shop->product()->pluck('id'))
                        ...
                        @php($end_date = date('Y-m-d', strtotime($shop['vacation_end_date'])))
                            <div class="col-lg-3 col-md-6 mb-1">
                                <a href="{{route('shopView', $shop['seller_id'])}}">
                                    <div class="lis-to-vendor">
                                        <div class="images-vendor">
                                            <img src="{{asset('storage/app/public/shop/'.$shop->image)}}"
                                                onerror="this.src='{{theme_asset('assets/img/image-place-holder.png')}}'"
                                             alt="{{$shop->name}}" loading="lazy" class="dark-support img-fit rounded-circle w-100">
                                        </div>
                                        <div class="text-vendor">
                                            <h6 class="fw-700"><b>{{Str::limit(ucwords($shop->name), 25)}}</b></h6>
                                            <div class=" ">
                                                <div class="start-rating-vendor">
                                                    <i class="fa-solid fa-star"></i> <span>{{ round($avg_rating ?? 0, 1) }}</span>
                                                </div>
                                                <div class="follower-rate-vendor">
                                                    @php($seller = \App\CPU\ProductManager::get_user_total_product('seller', $shop->seller_id))
                                                    <b>{{ $seller }}</b><span> {{ translate('products') }}</span>
                                                </div>
                                                <div class="follower-rate-vendor">
                                                    @php($count_follower=\App\Model\ShopFollower::where('shop_id',$shop->id)->count())
                                                    <b>{{ $count_follower ?? 0 }}</b> <span>{{ translate('follower') }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
