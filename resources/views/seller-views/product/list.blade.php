                        ...
                        <table id="datatable" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                            ...
                            @foreach($products as $k=>$p)
                                <tr>
                                        ...
                                        <div style="display: flex; gap: 10px; align-items: center;">
                                            ...
                                            <div class="modal fade" id="mrpEditModal{{ $p['id'] }}" tabindex="-1" role="dialog"
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <form action="{{ route('seller.product.update-price', [$p['id']]) }}" method="post"
                                    ...
                                    <td>{{ $p->package_weight ?? 'not calculated' }}</td>
                                    <td class="text-right">
                                        <div style="display: flex; gap: 10px; align-items: center;">
                                            {{ $p['current_stock'] }}
                                            <span class="btn btn-outline-primary btn-sm square-btn"
                                    ...
                                    <td class="text-right">
                                        ...
                                        <form id="outOfStockForm{{ $p['id'] }}"
                                            action="{{ route('seller.product.update-price', [$p['id']]) }}"
                                            method="POST"
                                            style="display: none;">
                                            @csrf
                                            <input type="hidden" name="type" value="quantity">
                                    ...
                                    <!-- <td>
                                        {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($p['unit_price']))}}
                                        <span  class="btn btn-outline-primary btn-sm square-btn"
                                            title="{{\App\CPU\translate('Edit_Price')}}" data-toggle="modal" data-target="#exampleModal{{$p['id']}}">
                                            <i class="tio-edit"></i>
                                        </span>

                                        <div class="modal fade" id="exampleModal{{$p['id']}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <form action="{{route('seller.profile.bank_update',[$p['id']])}}" method="post"
                                                    enctype="multipart/form-data">
                                                @csrf
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            ...
                                                            <div class="form-group">
                                                                <div class="row">
                                                                    <div class="col-md-6 mb-3">
                                                                        <label class="mt-2" for="old_price" class="title-color">{{\App\CPU\translate('Old Price')}}</label>
                                                                        <input type="text" name="old_price" value="{{\App\CPU\BackEndHelper::usd_to_currency($p['unit_price'])}}"
                                                                            class="form-control" id="old_price" readonly>
                                                                    </div>
                                                                    <div class="col-md-6 mb-3">
                                                                        <label class="mt-2" for="new_price" class="title-color">{{\App\CPU\translate('New Price')}}</label>
                                                                        <input type="text" name="new_price"
                                                                            class="form-control" id="new_price" required>
                                                                    </div>
                                                                </div>
