        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
            <tr>
                ...
                <td style="width: 50%; vertical-align: top; padding: 8px;text-align:right">
                    ...
                    <!-- <div>
                        <img src="{{ asset('storage/app/public/shop/' . $seller_data->shop->image) }}" alt="" class="vendorimg">
                    </div> -->
                </td>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
            <tr>
                <td style="width: 50%; vertical-align: top; padding: 8px;">
                    ...
                    <h5>{{ optional($seller_data->shop)->name ?? 'N/A' }}</h5>
                    <p>{{ optional($seller_data->shop)->address ?? 'N/A' }}</p>
                    <p>{{ optional($seller_data->shop)->gst_in ?? 'N/A' }}</p>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
            <tr>
                ...
                <td style="width: 20%; text-align: right; vertical-align: top; padding: 0;">
                    <img src="{{ asset('storage/app/public/seller/' . $seller_data->upi_scanner) }}" alt="QR Code" style="width: 100px; height: 100px;">
                </td>
            </tr>
        </table>
    </div> -->
            ...
            <td style="width: 40%; vertical-align: top; padding: 8px;">
                @if ($order->shippingAddress)
                    ...
                    <p>
                        ...
                        {{ $order->customer != null ? $order->customer['email'] : \App\CPU\translate('email_not_found') }}<br>
                ...
                @else
                    ...
                    <br>
                    <p>
                        <strong>{{ \App\CPU\translate('customer_info') }}</strong><br>
                        {{ $order->customer != null ? $order->customer['f_name'].' '.$order->customer['l_name'] : \App\CPU\translate('name_not_found') }}<br>
                        @if (isset($order->customer) && $order->customer['id'] != 0)
                            {{ $order->customer['email'] ?? \App\CPU\translate('email_not_found') }},
                            {{ $order->customer['phone'] ?? \App\CPU\translate('phone_not_found') }},
                            {{ $order->billingAddress['country'] ?? 'N/A' }}
                        @endif
                    </p>
                @endif
                    ...
                    <tr>
                        ...
                        <td style="text-align:left; white-space: nowrap; padding: 5px; vertical-align: top;">
                            <div style="display:flex; align-items:flex-start; gap:8px;">
                                @if(optional($details->product)->thumbnail)
                                    <img src="{{ asset('storage/app/public/product/'.optional($details->product)->thumbnail) }}"
                                        alt=""
                                        width="30"
                                        style="height:auto;">
                                @endif
                                <span>
                                    ...
                                    @if($details->variant)
                                        <br><strong>{{ \App\CPU\translate('variation') }}:</strong> {{ $details->variant }}
                                    @endif
                                    <br><strong>MRP:</strong>
                                    {{ \App\CPU\BackEndHelper::set_symbol(
                                        \App\CPU\BackEndHelper::usd_to_currency(optional($details->product)->unit_price ?? 0)
                                    ) }}
                                    @if(optional($details->product)->hsn_code)
                                        <br><strong>HSN:</strong> {{ optional($details->product)->hsn_code }}
                                    @endif
                                </span>
                            </div>
                        </td>
                        <td class="tablerowdiv">
                            {{ $details['tax'] > 0 && $line_total > 0 ? round(($details['tax']/$line_total)*100,2).'%' : '-' }}
                        </td>
                        <td class="tablerowdiv">{{ $details->qty }}</td>
                        <td class="tablerowdiv">{{ optional($details->product)->unit ?? '' }}</td>
                        <td class="tablerowdiv">
...
<div class="content-position-y">
    ...
    <!-- <div class="">
        <div class="content-position-y">
            <table class="customers bs-0" border="1" cellspacing="0" cellpadding="6" style="border-collapse:collapse; width:100%; text-align:center;">
                <thead>
                    <tr>
                        <th style="width: 50px;">{{\App\CPU\translate('SL')}}</th>
    ...
    <!-- <div class="content-position-y hdntable">
        ...
        <p><strong>Tax Amount (in words):</strong> NINETY-FOUR RUPEES ONLY</p> -->
  <div class="content-position-y hdntable">
            ...
            <tr>
                <!-- <td style="width: 40%; vertical-align: top;">
                  ...
                  <td style="width: 40%; vertical-align: top;">
                    ...
                    Company’s Name: {{ optional($seller_data->shop)->name ?? 'N/A' }}<br>
                    A/c Holder's Name:     {{ $order->customer != null ? $order->customer['f_name'].' '.$order->customer['l_name'] : \App\CPU\translate('name_not_found') }}<br>
                    Bank Name: {{ optional($seller_data)->bank_name ?? 'N/A' }}<br>
                    Branch IFSC Code: {{ optional($seller_data)->ifsc_code ?? 'N/A' }}<br>
                    UPI ID: {{ optional($seller_data)->upi_id ?? 'N/A' }}
                </td>
                <td style="width: 20%; text-align: right; vertical-align: top; padding: 8px;">
                    <img src="{{ asset('storage/app/public/seller/' . $seller_data->upi_scanner) }}" alt="" style="width: 100px; height: 100px;">
                </td>
                <!-- <td style="width: 20%; text-align: right; vertical-align: top; padding: 8px;">
