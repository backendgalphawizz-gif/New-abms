                            ...
                            <div class="text-sm-right">
                                ...
                                <div class="d-flex flex-column gap-2 mt-3">
                                    ...
                                    <!-- @if($order->payment_method != 'cash_on_delivery' && $order->payment_method != 'pay_by_wallet')
                                        <div class="reference-code -none d-flex justify-content-sm-end gap-10 text-capitalize">
                    ...
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <div>
                            ...
                            <select name="order_status" onchange="order_status(this.value)" class="status form-control" data-id="{{$order['id']}}">
                                ...
                                <!-- <option value="processing" {{$order->order_status == 'processing'?'selected':''}} >{{\App\CPU\translate('Packaging')}} </option> -->
                                <option value="shipped" {{$order->order_status == 'shipped'?'selected':''}} >{{\App\CPU\translate('Shipped')}} </option>
                                
                                <option value="out_for_delivery" {{$order->order_status == 'out_for_delivery'?'selected':''}} >{{\App\CPU\translate('out_for_delivery')}} </option>
                                <option value="delivered" {{$order->order_status == 'delivered'?'selected':''}} >{{\App\CPU\translate('Delivered')}} </option>
                                <option value="returned" {{$order->order_status == 'returned'?'selected':''}} > {{\App\CPU\translate('Returned')}}</option>
                                <option value="failed" {{$order->order_status == 'failed'?'selected':''}} >{{\App\CPU\translate('Failed_to_deliver')}} </option>
                                <option value="canceled" {{$order->order_status == 'canceled'?'selected':''}} >{{\App\CPU\translate('Canceled')}} </option>
                            </select>
                        </div>
                        ...
                        <ul class="list-unstyled">
                            ...
                            <li class="choose_delivery_man mt-3 d-none">
                                ...
                                <input type="number" id="deliveryman_charge" onkeyup="amountDateUpdate(this, event)" value="{{ $order->deliveryman_charge }}" name="deliveryman_charge" class="form-control" placeholder="Ex: 20" >
                            </li>
                            <li class="choose_delivery_man mt-3 d-none">
                                <label class="font-weight-bold title-color fz-14">
                                    {{\App\CPU\translate('expected_delivery_date')}}
                                </label>
                                <input type="date" onchange="amountDateUpdate(this, event)" value="{{ $order->expected_delivery_date }}" name="expected_delivery_date" id="expected_delivery_date" class="form-control" >
                            </li>

                            <li class="d-none mt-3" id="by_third_party_delivery_service_info ">
                                <span>
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-column gap-2">
                                ...
                                <div>
                                    ...
                                    <strong>{{$shipping_address ? $shipping_address->address.',' : ''}} {{$shipping_address && $shipping_address->address1 != "" ? $shipping_address->address1.',' : ''}} {{$shipping_address ? $shipping_address->city.',' : ''}} {{$shipping_address ? $shipping_address->state.',' : ''}} {{$shipping_address ? $shipping_address->country : ''}}</strong>
                                </div>
                                <div>
